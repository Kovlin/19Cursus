...
def init(request):
	try:
		...
		query = """ CREATE TABLE IF NOT EXISTS ex06_movies (
		...
		query = """CREATE OR REPLACE FUNCTION update_changetimestamp_column()
				RETURNS TRIGGER AS $$
				BEGIN
				NEW.updated = now();
...
def populate(request):
	...
	try:
		...
		query = """ INSERT INTO ex06_movies (episode_nb, title, director, producer, release_date) 
...
def update(request):
	try:
		...
		if request.method == 'POST':
			query = """	UPDATE ex06_movies
						SET opening_crawl = '{op}'
						WHERE episode_nb = {id};
					""".format(id = request.POST['Key'], op = request.POST['op'].replace("'"," "))
			cursor.execute(query)
			conn.commit()
		query = "SELECT * FROM ex06_movies"
		cursor.execute(query) #used to execute a query in a the database
