...
/*   Updated: 2020/10/30 17:35:25 by rlinkov          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int     m_close(int keycode, t_mlx *vars)
{
    if (keycode == 12)
    {
...
int     draw(int keycode, t_mlx *mlx)
{
...
//    mlx_loop_hook(&tmlx.mlx, key_hook, &tmlx);
    ...
    mlx->cube = cube;
    mlx->mlx = mlx_init();
    mlx->win = mlx_new_window(mlx->mlx, mlx->cube->res[X], mlx->cube->res[Y], "Cub3d");
